# Player Back Key Intercept

## Purpose

Ensure the Back key immediately dismisses player UI (control overlay and side sheets) on all device types by intercepting Back at the root `onKeyEvent` level, bypassing platform-specific `OnBackPressedDispatcher` behavior differences between TV and tablet Compose implementations.
